...
(function()
{
	// Capsule Shape, supports size style
	function CapsuleShape()
	{
		mxActor.call(this);
	};
	mxUtils.extend(CapsuleShape, mxActor);
	CapsuleShape.prototype.size = 20;

	CapsuleShape.prototype.redrawPath = function(path, x, y, w, h)
	{
		path.moveTo(h / 2, 0);
		path.lineTo(w - h / 2, 0);
		path.arcTo(h / 4, h / 4, 0, 0, 1, w - h / 2, h);
		path.lineTo(h / 2,  h);
		path.arcTo(h / 4, h / 4, 0, 0, 1, h / 2, 0);
		path.close();
		path.end();
	};

	mxCellRenderer.prototype.defaultShapes['capsule'] = CapsuleShape;
